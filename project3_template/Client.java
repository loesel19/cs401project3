    ...
    public void read_config_from_file(File file) {
        try {
            ...
            while (scanner.hasNextLine()) {
                ...
                switch (opt[0]) {
                    case "SERVERPORT":
                        serverPort = Integer.parseInt(opt[1]);
                        break;
                    case "CLIENTID":
                        peerID = Integer.parseInt(opt[1]);
                        break;
                    case "MYPORT":
                        peer_listen_port = Integer.parseInt(opt[1]);
                        break;
                    case "FILE_VECTOR":
                        FILE_VECTOR = opt[1].toCharArray();
                        break;
                }
            }
...
class PacketHandler extends Thread {
     ...
     * if the event type is 4. It also checks for quit packets from the peer and closes the connection
     * if one is received.
     *
     * @param p
     * @param i
    ...
    class FileReceiver extends Thread {
        ...
        public void run() {
            super.run();
            try {
                PeerToPeerHandler(p.peerIP, p.peer_listen_port, p.req_file_index, p.req_file_index, p.fileHash);
